...
type
  ...
  public
    property Items[const Index: integer]: TRedisValue read GetItems;
...
type
  ...
  public                 //发出指令后，就会接收数据，ReceiveTimeout 指最长等待多久
...
constructor TRedisValue.Create;
begin
...
constructor TBaseRedisSimpleClient.Create;
begin
  FReceiveTimeout := 500;
  FCmd := TRedisCommand.Create;
...
begin
  result := Append( bytesof( Key), BytesOf( value));
end;
...
constructor TRedisSimpleClient.Create;
begin
  inherited Create;
  FAutoConnect := true;
  FResValue :=  TRedisValue.create;
end;
...
begin
  result := HMGet( BytesOf( key), ArrStrToArrBytes( Fields));
end;
...
begin
  result := HMSet(bytesOf( key), ArrStrToArrBytes( FieldValues));
end;
...
begin
  result := HSET( bytesOf( key), bytesOf( Field), bytesOf( value));
end;

function TRedisSimpleClient.HGet(const Key, Field: string): boolean;
begin
  result := hGet( bytesOf( key), bytesOf( field));
end;
...
begin
  result := LSET(ListKey, Index, BytesOf( Value));
end;
...
function TRedisSimpleClient.Set_(const key, value: string;
  ASecsExpire: NativeInt): boolean;
begin
  result := set_( bytesOf( key), bytesof( value), ASecsExpire);
end;

function TRedisSimpleClient.StrLen(const Key: string): boolean;
begin
  result := StrLen(BytesOf( Key));
end;
...
constructor TSafeRedisSimpleClient.Create;
begin
  inherited Create;
